...
def read_g2o_file(filename: str) -> Tuple[List[RelativePoseMeasurement], int]:
    ...
    Returns:
        ...
        int: The number of poses.
        List[Edge]: The list of edges in the graph
    """
    measurements = []
    with open(filename, 'r') as infile:
        ...
        for line in infile:
            ...
            if (token == "EDGE_SE3:QUAT"):
                ...
                # mac_measurement = RelativePoseMeasurement(i=i,
                #                                       j=j,
                #                                       t=t,
                #                                       R=R,
                #                                       tau=tau,
    ...
    return measurements, num_poses, rpm_to_nx(measurements)
...
def rpm_to_mac(measurements: List[Measurement]) -> List[Edge]:
    ...
    Args:
        measurements (List[Measurement]): the list of relative pose measurements
    ...
    for meas in measurements:
        edge = Edge(meas.i, meas.j, meas.weight)
        edges.append(edge)
    return edges

def rpm_to_nx(measurements: List[Measurement]) -> nx.Graph:
    ...
    G = nx.Graph()
    for meas in measurements:
        G.add_edge(meas.i, meas.j, weight=meas.weight)
    return G

if __name__ == "__main__":
    test = read_g2o_file('mac_dataset/intel.g2o')
    print(test)
